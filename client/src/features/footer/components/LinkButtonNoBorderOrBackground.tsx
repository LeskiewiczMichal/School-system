...
export default function LinkButtonNoBorderOrBackground(
  ...
  return (
    <Link
      to={link}
      className="flex items-center justify-between gap-3 w-full border-b border-white bg-brandMainSoft text-center text-white font-bold text-md py-1"
    >
      {text} <ArrowRight className="w-10 h-10" />
    </Link>
  );
}

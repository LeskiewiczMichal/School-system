...
public class TestAssertions {
  ...
  //region User
  public static void assertUserInCollection(ResultActions matchers, int index, long id,
      String firstName, String lastName, String email, String faculty, String degree)
      throws Exception {
  ...
  public static void assertUser(ResultActions matchers, long id, String firstName, String lastName,
    ...
    matchers.andExpect(jsonPath(String.format("$.id")).value(faculty.getId()))
        ...
        .andExpect(
            jsonPath(String.format("$._links.self.href")).value(WebMvcLinkBuilder.linkTo(
                methodOn(FacultyController.class).getFacultyById(faculty.getId())).toString()));
  }
  //

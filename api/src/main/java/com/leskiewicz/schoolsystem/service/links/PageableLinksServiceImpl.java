...
public class PageableLinksServiceImpl implements PageableLinksService {
  ...
  @Override
  public <T, D> void addLinks(
      CollectionModel<D> resources,
      Page<T> page,
  ...
  public <T, D> void addLinks(
    ...
    UriComponentsBuilder linkBuilder =
        linkTo(controllerClass).slash(requestRelativePath).toUriComponentsBuilder();
    addLinksHelper(resources, page, request, linkBuilder);
  }

  private <T, D> void addLinksHelper(
      CollectionModel<D> resources,
      Page<T> page,
      PageableRequest request,
      UriComponentsBuilder linkBuilder) {
